...
async function waitForDatabase(pool: InstanceType<typeof Pool>, maxRetries = 5): Promise<void> {
...
export async function buildApiServer(config: ApiServerConfig) {
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    const isLocalhost = process.env.NODE_ENV !== "production" && origin !== undefined && /^http:\/\/localhost(:\d+)?$/.test(origin);
