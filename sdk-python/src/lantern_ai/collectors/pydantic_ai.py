...
class LanternPydanticAIHandler:
    """Callback handler for Pydantic AI agent lifecycle events."""

    def __init__(self, tracer: "LanternTracer", *, agent_name: Optional[str] = None):
        self._tracer = tracer
        self._agent_name = agent_name or "pydantic-ai-agent"
    ...
    def on_step(self, step_name: str = "", **kwargs: Any) -> str:
        tid = self._ensure_trace()
        span = self._tracer.start_span(tid, type="reasoning_step", input=SpanInput(prompt=step_name))
        return span.id
...
def create_lantern_pydantic_handler(tracer: "LanternTracer", *, agent_name: Optional[str] = None) -> LanternPydanticAIHandler:
    """Create a Lantern handler for Pydantic AI instrumentation."""
    return LanternPydanticAIHandler(tracer, agent_name=agent_name)
